...
use crate::{
    pe::Pe,
    report::{CodeTransform, RecoveredProgram, StartupKind},
};
...
pub(crate) fn select_output_entry(mapped: &mut [u8], pe: &Pe) -> Result<SelectedOutputProfile> {
    ...
    if pe.is_dll() || has_nonempty_com_descriptor(pe) {
        return raw
            .map(|entry| SelectedOutputProfile {
                entry,
    ...
    for page_key in page_keys {
        ...
        match result {
            Ok(entry) => successes.push((page_key, entry)),
            Err(error) => failures.push(format!("{page_key:?}: {error:#}")),
        }
    }
