...
fn managed_bootstrap_discovery() -> LoaderDiscovery {
    LoaderDiscovery {
        table_rva: 0x4000,
        metadata_ranges: vec![0x4000..0x4080],
        image_size: 0x6000,
        modules: vec![crate::unpack::imports::ImportModule {
            ...
            .into_iter()
            .map(|name| crate::unpack::imports::ImportSymbol::Name {
                hint: 0,
                name: name.into(),
            })
            .collect(),
...
#[test]
fn managed_konn_bootstrap_residue_is_a_pe_rebuild_boundary() {
    let pe = synthetic_pe(0);
    let mut image = vec![0; 0x6000];
    image[usize::try_from(MANAGED_RESIDUE_RECOGNIZER_RVA).unwrap()
        ..usize::try_from(MANAGED_RESIDUE_RECOGNIZER_RVA).unwrap()
            + MANAGED_RESIDUE_RECOGNIZER.len()]
        .copy_from_slice(&MANAGED_RESIDUE_RECOGNIZER);
    let discovery = managed_bootstrap_discovery();
    let error = reject_unproven_managed_residue(
        &image,
        &pe,
        OutputEntry::Managed { entry_rva: 0 },
        &discovery,
    )
    .unwrap_err()
    .to_string();
    assert!(
        error.contains("original managed bootstrap/import/relocation provenance is unavailable")
    );
    let mut report = AnalysisReport::default();
    report.completed(crate::report::AnalysisStep::ImportRecovery);
    assert!(
        finish_stage(
            &mut report,
            crate::report::AnalysisStep::PeRebuild,
    ...
    assert_eq!(report.status, crate::report::AnalysisStatus::Failed);
    assert_eq!(
        report.last_completed_step,
        Some(crate::report::AnalysisStep::ImportRecovery)
    );
    assert_eq!(
        report.error.unwrap().step,
        crate::report::AnalysisStep::PeRebuild
    );
}

#[test]
fn managed_residue_near_misses_do_not_reject() {
    let pe = synthetic_pe(0);
    let mut image = vec![0; 0x6000];
    let start = usize::try_from(MANAGED_RESIDUE_RECOGNIZER_RVA).unwrap();
    image[start..start + MANAGED_RESIDUE_RECOGNIZER.len()]
        .copy_from_slice(&MANAGED_RESIDUE_RECOGNIZER);
    let discovery = managed_bootstrap_discovery();
    let managed = OutputEntry::Managed { entry_rva: 0 };

    let mut wrong_opcode = image.clone();
    wrong_opcode[start] ^= 1;
    assert!(reject_unproven_managed_residue(&wrong_opcode, &pe, managed, &discovery).is_ok());

    let mut wrong_dll = discovery.clone();
    wrong_dll.modules[0].dll = "user32.dll".into();
    assert!(reject_unproven_managed_residue(&image, &pe, managed, &wrong_dll).is_ok());

    let mut wrong_order = discovery.clone();
    wrong_order.modules[0].symbols.swap(0, 1);
    assert!(reject_unproven_managed_residue(&image, &pe, managed, &wrong_order).is_ok());

    let mut ordinal = discovery.clone();
    ordinal.modules[0].symbols[0] = crate::unpack::imports::ImportSymbol::Ordinal(1);
    assert!(reject_unproven_managed_residue(&image, &pe, managed, &ordinal).is_ok());

    let mut extra_module = discovery.clone();
    extra_module.modules.push(extra_module.modules[0].clone());
    assert!(reject_unproven_managed_residue(&image, &pe, managed, &extra_module).is_ok());

    let mut non_executable = pe.clone();
    non_executable.sections[0].characteristics &= !IMAGE_SCN_MEM_EXECUTE;
    assert!(reject_unproven_managed_residue(&image, &non_executable, managed, &discovery).is_ok());
}
