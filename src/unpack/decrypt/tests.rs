...
fn decryption_replay_budget_is_independent_per_candidate_chain() {
    ...
    assert_eq!(decryption_details.candidate_combinations_tested, 2);
    assert_eq!(decryption_details.candidate_combinations_rejected, 1);
    assert_eq!(
        decryption_details.selected_byte_transform,
        Some(crate::report::ByteTransform::FixedF8)
    );
    assert_eq!(decryption_details.sample_rejections.len(), 1);
    assert_eq!(decryption_details.sample_rejections[0].chunk_index, 1);
}
...
fn decryption_selection_failure_reports_bounded_custom_rejections() {
    ...
    assert_eq!(decryption_details.byte_transform_candidates, 1);
    assert_eq!(decryption_details.candidate_combinations_tested, 9);
    assert_eq!(decryption_details.candidate_combinations_rejected, 9);
    assert_eq!(decryption_details.selected_byte_transform, None);
    assert_eq!(decryption_details.sample_rejections.len(), 8);
    assert!(
        decryption_details
            .sample_rejections
            .iter()
            .all(|rejection| rejection.chunk_index == 1)
    );
    assert!(
        decryption_details
            .sample_rejections
            .iter()
            .all(|rejection| rejection.reason
                == CustomDecodeError::InvalidRepeatWidth { width: 3 }.to_string())
    );
}
